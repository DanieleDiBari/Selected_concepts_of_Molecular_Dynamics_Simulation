...
\begin{frame}[label=intro-why_use_CS]
	...
	This method of screening theories before we apply them to the real world is called a \textit{computer experiment}.}
\end{frame}
...
\begin{frame}[label=intro-virtual_lab]
	...
	Technologically useful informations, for systems that may be difficult or even impossible to investigate directly with a real experiment
	(e.g. nuclear reactors or planetary cores),
	can be obtained from those type of simulations. 
...
\begin{frame}[label=intro-diff_typesd]
	...
	\footnotetext[1]{When the outcome of a random event in a sequence depends only on the outcome of the immediately previous one, 
	the sequence is called: \textit{Markov chain}.}
\end{frame}

\begin{frame}[label=intro-diff_typesd]
	\frametitle{Introduction}
	\framesubtitle{A brief view of Monte Carlo Simulations}
	\justifying
