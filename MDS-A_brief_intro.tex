...
\begin{frame}
	...
	\framesubtitle{Why do we should use Computer Simulation?}
	\justifying
	Computer simulations allow us to calculate the essentially exact results for a given model system without having to rely on approximate theories. 

	We can use the results of a simulation to compare them with:
	
	\begin{itemize}\justifying
		\item[$\blacktriangleright$] the results of an experiment - TESTS OF MODELS;
		\item[$\blacktriangleright$] the predictions of an approximate analytical theory applied to the same model - TESTS OF THEORY.
	\end{itemize}	
	
	If we found some disagrees in the two cases seams respectively that:
	
	\begin{itemize}\justifying
		\item[$\blacktriangleright$] our model is inadequate - we have to improve on our estimate of the intermolecular interactions;
		\item[$\blacktriangleright$] the theory is flawed. The computer simulation plays the role of the experiment designed to test the theory. This method of screening theories before we apply them to the real world is called a computer experiment.
	\end{itemize}	
	 or that (
\end{frame}
