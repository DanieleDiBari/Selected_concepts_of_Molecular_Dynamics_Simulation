...
\begin{frame}[label=MDS-velVerl2]
	...
	Thus, the steps \textbf{velocity Verlet algorithm} is:
...
\begin{frame}[label=MDS-evOperator]
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{The classical time evolution operator}
	\justifying
...
\begin{frame}[label=MDS-evOperator]
	...
	\small{	
	\begin{align*}
		&p[i] = p[i] + 0.5 \cdot \Delta t \cdot F[i]\\
		&r[i] = r[i] + \Delta t \cdot p / m[i]\\
		&\text{\textit{Recalculate the force}}\\
		&p[i] = p[i] + 0.5 \cdot \Delta t \cdot F[i]
	\end{align*}
	}
	Note that \textbf{these are just the steps required by the operator factorization scheme of \eqref{eq:trotter}}. 
	The first operator that acts on the phase space produces the momentum translation; 
	the next operator takes the output of the preceding step and performs the position translation; 
	since this step changes the positions, the force must be recalculated; 
	the last operator produces the final momentum translation using the new force. 
...
\begin{frame}[label=MDS-evOperator]
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{Summary of the numerical integrators}
	\justifying
