...
\begin{frame}[label=MDS-integrators1]
	\frametitle{Molecular Dynamics Simulations}
...
\begin{frame}
	...
	\vspace*{-0.1cm}
	A good MD program requires a good algorithm to integrate the equations of motion.	
	In this sense, \textbf{the choice of the numerical integrator employed is crucial}. 
	However, although it is easy to recognize a bad integrator, it is not obvious what criteria a good integrator should satisfy. 
	Indeed there are different points to consider.
	...
	\begin{itemize}\justifying
		...
		\item	\bluetextbf{Accuracy for large $\Delta t$ is more important}: the bigger is the $\Delta t$ used, the fewer are evaluations of the 
				forces needed. Hence, it is useful to employ a sophisticated algorithm that allows use of a long $\Delta t$.\footnotemark[1]
	\end{itemize}
	
	\footnotetext[1]{\justifying Actually exist some algorithms that, storing information on increasingly higher-order derivatives of the particle coordinates, allow to use of large $\Delta t$. They require more memory storage, but typically this is not a serious drawback.}
...
\begin{frame}
	...
	\footnotetext[1]{For instance the predictor-corrector schemes and many of the schemes used to deal with constraints.}
...
\begin{frame}
	...
	However, this not involve that the reversibility alone is sufficient to guarantee the absence of such energy drift. 
...
\begin{frame}
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{Finite-difference method - I}
	\justifying
	...
	\begin{block}{The Finite-Difference Method - FDM}\justifying
		The central idea of this approach is to \textbf{approximate the derivatives by differences} and thereby reduce differential equations to
...
\begin{frame}
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{Finite difference method - II}
	\justifying
...
\begin{frame}
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{Finite difference method - III}
	\justifying
...
\begin{frame}
	...
	The terms truncation and round-off refer to sources of errors; however, it should be considered not only what causes errors but also how errors propagate - whether they grow as the simulations proceeds. This is the issue of algorithmic stability.
\end{frame}
