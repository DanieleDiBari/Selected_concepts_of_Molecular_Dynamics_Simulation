...
\begin{frame}[label=intro-molecular_sim1]
	\frametitle{Introduction}
	\framesubtitle{Molecular simulations}
	\justifying
...
\begin{frame}[label=intro-principal_steps]
	\frametitle{Introduction}
...
\begin{frame}[label=intro-diff_typesd]
	\frametitle{Introduction}
	\framesubtitle{Different types of Computer Simulations}
	\justifying
...
\begin{frame}[label=intro-MCSeMDS]
	\frametitle{Introduction}
	\framesubtitle{The main differences between Molecular Dynamics and Monte Carlo - I}
	\justifying
		
%	Molecular dynamics has the advantage of allowing the study of time-dependent processes, 
%	but for the calculation of static properties a Monte Carlo method may be more efficient. 
	
%	Although the physical and mathematical basis of Monte Carlo may be less transparent to a novice than that for molecular dynamics, 
	Monte Carlo is usually easier to code in a high-level language and, in general, to implement when the 
	intermolecular force law is difficult to extract from the potential function.
	
	\vspace*{0.3cm}
	
	For the determination of equilibrium properties, the two methods are equally effective
	- they require about the same amount of computing time to attain similar levels of statistical precision. 
	
	\vspace*{0.3cm}
	
	Molecular Dynamics has the advantage of allowing the study of time-dependent processes such as transport coefficients and time 
	correlation functions that cannot generally be obtained by Monte Carlo.\footnotemark[1]
		
	\footnotetext[1]{Although certain kinds of dynamic behaviour may be deduced from Monte Carlo simulation.}

\end{frame}

\begin{frame}[label=intro-MCSeMDS]
	\frametitle{Introduction}
	\framesubtitle{The main differences between Molecular Dynamics and Monte Carlo - II}
	\justifying
	
	Molecular Dynamics also offers certain  \underline{computational advantages}
	\underline{because of the deterministic way in which it generates trajectories}.

	\vspace*{0.3cm}
	
	The presence of an explicit time variable allows to \textbf{roughly estimate the length needed for the run} -
	al least several multiples of the relaxation time (depending on the number of particles). 
	\textbf{No such convenient guide is available for a Monte Carlo calculation}. 

	\vspace*{0.3cm}
		
	Finally, in a Molecular Dynamics program, small errors tend to accumulate with time involving clear violations of conservation principles 
	that allow to reveal them. 
	On the contrary, in a Monte Carlo program this does not happen, thus small errors may be not easy to reveal.
%	Finally, many kinds of small errors in a Molecular Dynamics program tend to accumulate with time and so become apparent as 
%	violations of conservation principles; in contrast, small errors in a Monte Carlo program may not clearly advertise their presence.
\end{frame}
...
\begin{frame}[label=intro-MCSeMDS]
	...
	The choice between Molecular Dynamics and Monte Carlo is largely determined by the phenomenon under investigation.
	
	\begin{itemize}\justifying\small{
		\item \textbf{GAS (}\textit{or other low density systems}\textbf{) - Monte Carlo is preferable:}\\
		...
		\item \textbf{LIQUIDS - Molecular Dynamics is preferable:}\\
		Collisions exchange energy between molecules, enabling barrier crossings, improving the ability of MD to sample conformations.
...
\begin{frame}[label=intro-MCSeMDS]
	...
	MD also handles collective motions better than MC. However, recently methods such as Configurational Bias Monte Carlo and Hybrid Monte Carlo have been developed to improve the performance of MC simulations. 
	
	There are some situations where only one method is appropriate. Determination of transport properties such as viscosity coefficients, is largely only possible using MD, as MC lacks an objective definition of time (except in some special cases such as the bond fluctuation model for polymers). On the other hand MC can be used for simulations with varying particle numbers (Grand Canonical Monte Carlo) by adding moves for the creation and destruction of particles. 

%[Dr S J Clark - http://cmt.dur.ac.uk/sjc/thesis_dlc/node58.html]
\end{frame}
	
\begin{frame}[label=intro-MCS1]
	\frametitle{Introduction}
...
\begin{frame}[label=intro-MCS3]
	\frametitle{Introduction}
	\framesubtitle{A summary of the Monte Carlo simulations: Metropolis method}
	\justifying	
