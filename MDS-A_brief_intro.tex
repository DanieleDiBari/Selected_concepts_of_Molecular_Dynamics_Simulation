...
\begin{frame}[label=MDS-ensemble1]
	...
	The microcanonical ensemble consists of all microscopic states on the constant energy hypersurface 
	...
	since these equations conserve the Hamiltonian $H ( \bold{q}^N, \bold{p}^N )$, a trajectory computed with them will generate 
	microscopic configurations belonging to a microcanonical ensemble with energy $E$.
...
\begin{frame}[label=MDS-ensemble2]
	...
	\begin{columns}
		...
		\begin{column}{0.46\textwidth}\justifying
			If a system with energy $E$ is able, after a long time, to visit  practically all the configurations on the constant energy hypersurface, 
	...
	\underline{provide a powerful method for generating an ensemble and} \underline{its averages}. 
...
\begin{frame}[label=MDS-integrators1]
	\frametitle{Molecular Dynamics Simulations}
	\framesubtitle{The main aspects}
	\justifying
	...
	\begin{enumerate}\justifying
		...
		\item \textbf{The numerical integrator used}, i.e. the algorithm employed to integrate the equations of motion
	\end{enumerate}	
	...
	This presentation focus on the problem of devising a numerical integrator or solver for the equations of motion.
