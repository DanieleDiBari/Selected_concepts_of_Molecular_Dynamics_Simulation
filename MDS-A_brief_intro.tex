...
\begin{frame}[label=MDS-FDM3]
	...
	where  $f'_C(x)$ is the \textbf{central difference} that involves an approximation of order $O(\Delta x^2)$ and thus, for small $\Delta x$, it is 
...
\begin{frame}[label=MDS-verlet2]
	...
	This algorithm offers the virtues of \textbf{simplicity and good stability for considerately large time steps}. However it treated velocities as 
	less important than positions - a view in conflict with the attitude that the phase space trajectory depends equally on them. 
...
\begin{frame}[label=MDS-verlet3]
	...
	\begin{block}{\small{The main drawback of the Verlet algorithm}}\justifying\small{
		...
		eq. \eqref{eq:taylor-verlet} can be used to obtain a set of coordinates, $\bold{r}_1(\Delta t), \dots , \bold{r}_N(\Delta t)$, after which it is 
		possible to use the Verlet integrator, eq. \eqref{eq:verlet}, to generate a trajectory of arbitrary length.
	}
	\end{block}
\end{frame}
...
\begin{frame}[label=MDS-leap1]
	...
	From the second equation it is possible to obtain an expression for the new positions, based on the old positions and velocities\footnotemark[2]:
	\begin{equation*}
...
\begin{frame}[label=MDS-leap2]
	...
	From the expansion that determinates the Verlet algorithm (eq. \ref{eq:verlet-exp}), one gets the following expression for the update of the 
	velocities:
	...
	\underline{the\textcolor{white}{y}velocities are} \underline{not defined at the same time as the positions}. Thus, kinetic and potential energy are also not defined at the same time, and 
	hence one cannot directly compute the total energy in the Leap Frog scheme.
\end{frame}
