...
\note{
	The most common application of computer simulations is to \textbf{predict the properties of materials} 
	(even those that have not yet been made).

	The obvious \textbf{downside} is that the results are only as good as the numerical model and the results can be artificially biased 
	if the simulation is unable to sample an adequate number of microstates over the time it is allowed to run.
}
...
\begin{frame}[label=intro-MCS3]
	\frametitle{Introduction}
	\framesubtitle{A summary of Monte Carlo Simulations: Metropolis method}
	\justifying
	
	The Metropolis method involves the following principal steps:
	
	\begin{enumerate}
		\item	Assign initial positions $\bold{r}_i$ to the $N$ molecules and compute the potential energy $U$.
		\item Hypothesize a new configuration by arbitrarily choosing one molecule and proposing that it be moved from $\bold{r}$ through a randomly chosen distance and direction to a new position $\bold{r'}$.
		\item Compute the total potential energy $U'$for this new configuration and accept the proposed move (allow it to occur) based on the following criteria:
		\begin{equation}
		\begin{cases}
			\text{if } U' < U \Longrightarrow \text{accept with probability} = 1\\
			\text{if } U' > U \Longrightarrow \text{accept with probability} \propto  e^{ -\beta \, (U' - U)} 
		\end{cases}	
		\end{equation}
